...
const aidTypesSlice = createSlice({
  ...
  reducers: {
    ...
    removeAidType: (state, action: PayloadAction<number>) => {
      state.aidTypes = state.aidTypes.filter((type: IAidType) => Number(type.id) !== action.payload);
    },
  },
});
...
export const getAidTypes = (token?: string) => async (dispatch: AppDispatch) => {
  ...
  try {
    ...
    const { data } = await axios.get<IAidType[]>(API_KEY + AID_TYPES_PATHS.GET_AID_TYPES, headers);
...
export const createAidTypeAction = (name: string, token: string) => async (dispatch: AppDispatch) => {
  dispatch(setLoading(true));
  dispatch(setError(null));
...
export const editAidTypeAction = (id: number, name: string, token: string) => async (dispatch: AppDispatch) => {
  dispatch(setLoading(true));
  dispatch(setError(null));
...
export const deleteAidTypeAction = (id: number, token: string) => async (dispatch: AppDispatch) => {
  dispatch(setLoading(true));
  dispatch(setError(null));
