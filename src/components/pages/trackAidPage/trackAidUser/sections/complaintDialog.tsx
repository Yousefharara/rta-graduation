...
interface ISendComplaintForm {
  subject: string;
  message: string;
}

const defaultValues: ISendComplaintForm = {
  subject: "",
  message: "",
};
...
const ComplaintDialog = ({ open = false, setOpen }: IPorps) => {
  ...
  const {
    ...
    reset,
    register,
  } = useForm<ISendComplaintForm>({
    resolver: yupResolver(schemaSendComplaintForm),
  });

  const messageError = errors["message"]?.message;

  const handleOnSubmit = async (data: ISendComplaintForm) => {
    if (!beneficiary) {
    ...
    if (beneficiary.status !== "eligible") {
      toast.error(
        "حسابك قيد المراجعة، لا يمكنك تقديم شكوى حتى يتم توثيق حسابك",
      );
      return;
    }
        ...
        >
          ...
          <DialogFooter className="flex items-center flex-wrap gap-2">
            <Button
              ...
              type="submit"
            >
              {isCreating ? "جاري الإرسال..." : "ارسال الشكوى"}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
