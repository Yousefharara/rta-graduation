...
const DashboardComplaintsTable = () => {
  ...
  const { complaints, isFetching, isUpdating, error } = useAppSelector(
  ...
  const columns = useMemo<ColumnDef<IComplaint>[]>(() => {
    return [
      ...
      },
      {
        header: "المستفيد",
        cell: ({ row }) => (
          <p>{getBeneficiaryName(row.original.beneficiary_id)}</p>
        ),
      },
      {
        header: "الموضوع",
  ...
  if (error)
    return <Error onRetry={() => dispatch(getComplaints(accessToken || ""))} />;
