...
const DashboardAidsTable = () => {
  ...
  const getAidTypeName = useCallback(
    (id: number) => {
  ...
  const getOrgName = useCallback(
    (id: number) => {
      ...
      console.log("org name in aids is ", o);
      return o?.users.name || `${id ? `#${id}` : "لا يوجد منظمة"}`;
    },
    [localOrgs],
  );
  ...
  const columns = useMemo<ColumnDef<IAid>[]>(() => {
    return [
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <p
            className={`font-semibold ${row.original.remaining_quantity <= 0 ? "text-red-600" : "text-green-600"}`}
          >
  ...
  if (error)
    return <Error onRetry={() => dispatch(getAids(accessToken || ""))} />;
