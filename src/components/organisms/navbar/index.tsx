  ...
  return (
    <nav className="w-full bg-white z-20 fixed top-0 backdrop-blur-sm mx-auto border-b border-b-gray-300 py-[.7rem] ">
      <Container className="flex items-center justify-between">
        ...
        <ul
          className={`z-50 ${isMobile ? "block" : "hidden"} cursor-pointer`}
          onClick={handleToggle}
        >
          <li>
            <Menu className={`${isOpen ? "text-white" : "text-primary"}`} />
          </li>
        </ul>
        ...
        >
          ...
          >
            {role === "guest" && (
              <>
                <li>
                  <NavLink onClick={handleClose} to={PATHS.AUTH.LOGIN}>
                    تسحيل الدخول
                  </NavLink>
                </li>
              </>
            ...
            {role === "guest" && (
              <li>
                <NavLink
                  ...
                  onClick={handleClose}
                  className={
                    "not-active text-white! rounded-md px-4 py-2 bg-blue-primary"
                  }
                >
                  تبرع الان
                </NavLink>
          ...
          >
            {(role === "guest" || role === "beneficiary") && (
              <>
                <li>
                  <NavLink onClick={handleClose} to={PATHS.CONTACT_US}>
                    تواصل معنا
                  </NavLink>
                </li>

                <li>
                  <NavLink onClick={handleClose} to={PATHS.TRACK_AID.ROOT}>
                    تتبع المساعدات
                  </NavLink>
                </li>
              </>
            ...
            <li>
              <NavLink onClick={handleClose} to={PATHS.ABOUT}>
                حول المنصه
              </NavLink>
            </li>
            <li>
              <NavLink onClick={handleClose} to={PATHS.HOME}>
                الرئيسية
              </NavLink>
            </li>
            {(role === "admin" || role === "local_org") && (
              <li>
                <NavLink onClick={handleClose} to={PATHS.DASHBOARD.ROOT}>
                  لوحة التحكم
                </NavLink>
              </li>
            )}
